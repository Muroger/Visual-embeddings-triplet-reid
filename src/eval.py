...
parser.add_argument(
    '--excluder', default='diagonal', choices=('market1501', 'diagonal','duke'),
...
def main():
    ...
    for start_idx, (batch_pids, batch_fids, batch_embs) in zip(count(step=args.batch_size), dataloader):
        ...
        mask = np.array(excluder(fids))
        distances = np.array(distances)
        distances[mask.astype(int)] = np.inf
        ...
        for i in range(len(distances)):
            ...
            dist = np.squeeze(distances[i])
            
            k = np.where(pid_matches[i, np.argsort(dist)])[0][0]
            cmc[k:] += 1
